...
-------------------------------- */
window.__wormSetMint = (m) => { window.TOKEN_MINT = m; LIVE_MINT = resolveMint(); };
window.__wormSetWorker = (b) => { window.WORKER_BASE = b; };



/* =========================
   LIVE MOMENTUM PATCH (SAFE)
   - Prevents blank screen
   - Keeps sim active until live data is confirmed
   - No existing mechanics removed
   ========================= */

window.__LIVE_STATE__ = {
  enabled: true,
  ready: false,
  lastSeenBuys: 0,
  momentum: 0,
...
async function __pollLiveStatsSafe() {
  try {
    const MINT = "6jKn8hRBvdHZ3AnVNECnKgnLn655AwyNF3EvH9uRpump";
    const ENDPOINT = "https://heliuswebhook.nf5w8v8d4k.workers.dev/stats?mint=" + MINT;
    const res = await fetch(ENDPOINT, { cache: "no-store" });
    if (!res.ok) return;
    const data = await res.json();
    if (!data || typeof data.totalBuys !== "number") return;

    window.__LIVE_STATE__.ready = true;

    const delta = data.totalBuys - window.__LIVE_STATE__.lastSeenBuys;
    if (delta > 0) {
      window.__LIVE_STATE__.momentum += delta;
      window.__LIVE_STATE__.lastSeenBuys = data.totalBuys;

      // Momentum hooks (non-destructive)
      // 1 buy = worm
      for (let i = 0; i < delta; i++) {
        try { spawnLiveWorm && spawnLiveWorm(); } catch {}
      }
    }
  } catch (e) {
    // swallow errors – NEVER blank the sim
  }
}

// soft polling, never blocks render loop
setInterval(__pollLiveStatsSafe, 6000);

// Fallback safety: if live never becomes ready, sim continues forever
setTimeout(() => {
  if (!window.__LIVE_STATE__.ready) {

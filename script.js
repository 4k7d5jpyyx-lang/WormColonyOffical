...
function updateBossCinematic(nowMs) {
  ...
  if (!head) { bossCam.active = false; return; }
  ...
  if (nowMs >= bossCam.end) {
    ...
    // restore previous focus state (but don't hard snap the camera)
...
function updateBossCamera(dt) {
  ...
  function drawBackground() {
    // Use integer placement + slight overlap to avoid hairline seams between tiles
    const rawX = (-camX * zoom * 0.10);
    const rawY = (-camY * zoom * 0.10);
    const px = ((rawX % bg.w) + bg.w) % bg.w;
    const py = ((rawY % bg.h) + bg.h) % bg.h;
    ...
    for (let ix = -1; ix <= 1; ix++) {
      for (let iy = -1; iy <= 1; iy++) {
        ctx.drawImage(bg.canvas, x0 + ix * bg.w, y0 + iy * bg.h, w, h);
      }
    }

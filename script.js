...
function updateBossCamera(dt) {
  ...
  function computeRarity(w) {
    ...
    // patterns (exclusive: take the strongest one only)
    ...
    else if (w.width > 7) score += 5;
    // speed (non-stacking)
    if (w.speed > 1.25) score += 6;
    else if (w.speed > 1.05) score += 4;
    if ((w.segs?.length || 0) > 20) score += 4;

    // limbs
    score += Math.min(16, (w.limbs?.length || 0) * 2);
    ...
    if (score >= 72) { tier = "MYTHIC"; emoji = "💠"; }
    else if (score >= 60) { tier = "LEGENDARY"; emoji = "🟡"; }
    else if (score >= 44) { tier = "EPIC"; emoji = "🟣"; }
...
let __sync = {
  ...
  function __applyState(s) {
    ...
    // Market stats are derived locally from DexScreener; only fall back to server values
    // if we haven't successfully fetched market data yet.
    if (!marketLastAt) {
...
// Update stats from the market feed (these CAN go down on a rolling 24h window)
// Tiny smoothing to avoid UI flicker on rapid small swings.
const SMOOTH = 0.35; // 0 = no smoothing, 1 = snap instantly
const lerp = (a, b, t) => a + (b - a) * t;
const smoothNum = (prev, next) => {
  if (!Number.isFinite(next)) return prev;
  if (!Number.isFinite(prev)) return next;
  return lerp(prev, next, SMOOTH);
};

if (typeof best.buys24h === "number" && Number.isFinite(best.buys24h)) {
  buyers = Math.max(0, Math.round(smoothNum(buyers || 0, best.buys24h)));
}
if (typeof best.volume24h === "number" && Number.isFinite(best.volume24h)) {
  volume = Math.max(0, smoothNum(volume || 0, best.volume24h));
}
if (typeof best.mcap === "number" && Number.isFinite(best.mcap)) {
  mcap = Math.max(0, smoothNum(mcap || 0, best.mcap));
}

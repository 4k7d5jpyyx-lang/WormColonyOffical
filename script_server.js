...
function updateBossCamera(dt) {
  ...
  // =========================
  const API_BASE = (window.WORM_API_BASE || "").replace(/\/$/, "");
  ...
  function __applyState(s) {
    ...
    nextSplitAt = Number(s.nextSplitAt) || MC_STEP;

    camX = Number.isFinite(s.camX) ? s.camX : 0;
    camY = Number.isFinite(s.camY) ? s.camY : 0;
    zoom = Number.isFinite(s.zoom) ? clamp(s.zoom, 0.55, 2.8) : zoom;

    selected = clamp(Number(s.selected) || 0, 0, Math.max(0, (s.colonies.length - 1)));
    focusOn = !!s.focusOn;
  ...
  async function loadWorldFromServer() {
    ...
    __sync.lastRevision = Number(body.revision) || __sync.lastRevision;

    const applied = __applyState(body.state);
    return { ok: applied, changed: applied };
  }
  ...
  async function saveWorldToServer(reason = "autosave") {
    ...
    try {
      ...
      const payload = {
        ...
        clientTime: Date.now(),
        reason,
        // pass revision so the backend can choose to ignore stale writes if it wants
        revision: __sync.lastRevision || 0,
      };
      ...
      if (body && body.revision) __sync.lastRevision = Number(body.revision) || __sync.lastRevision;

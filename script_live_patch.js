...
function updateBossCamera(dt) {
  ...
  canvas.addEventListener("touchmove", (e) => {
    ...
    zoom = clamp(pinchStartZoom * scale, 0.55, 2.8);
...
// =============================
// LIVE ON-CHAIN BUY POLLING (non-breaking add-on)
// =============================
// Updates the existing `buyers` variable from your Cloudflare Worker.
// Keeps your mechanics intact (growthScore/events/UI keep using `buyers`).

const LIVE_WORKER_BASE = "https://heliuswebhook.nf5w8v8d4k.workers.dev";
const LIVE_TOKEN_MINT = "6jKn8hRBvdHZ3AnVNECnKgnLn655AwyNF3EvH9uRpump";

// How often to poll the worker (ms)
const LIVE_POLL_MS = 5000;

// If true, prints live polling info in console
const LIVE_DEBUG = false;

let __liveLastTotalBuys = null;

// Try multiple endpoints (because your worker has changed behavior during setup)
async function __fetchLiveStats() {
  const endpoints = [
    // common patterns
    `${LIVE_WORKER_BASE}/stats?mint=${encodeURIComponent(LIVE_TOKEN_MINT)}`,
    `${LIVE_WORKER_BASE}/?mint=${encodeURIComponent(LIVE_TOKEN_MINT)}`,
    `${LIVE_WORKER_BASE}?mint=${encodeURIComponent(LIVE_TOKEN_MINT)}`,
  ];
  ...
  for (const url of endpoints) {
    try {
      ...
      // some responses are plain "ok" or "OK"
      ...
      // If it wasn't JSON, keep trying.
      lastErr = new Error(`Non-JSON response from ${url}: ${text.slice(0, 80)}`);
    } catch (e) {
      lastErr = e;
    }
  }
  throw lastErr || new Error("Unable to fetch live stats");
}

function __applyLiveStats(stats) {
  // Expected: { totalBuys, recentBuys, lastBuyAt, ... }
  const totalBuys = Number(stats?.totalBuys);

  if (!Number.isFinite(totalBuys)) {
    if (LIVE_DEBUG) console.warn("[LIVE] stats missing totalBuys", stats);
    return;
  }

  // Only move forward (never decrease buyers)
  if (typeof buyers === "number") {
    if (totalBuys > buyers) {
      buyers = totalBuys;
    }
  }

  __liveLastTotalBuys = totalBuys;

  if (LIVE_DEBUG) {
    console.log("[LIVE] totalBuys =", totalBuys, "buyersVar =", buyers);
  }
}

async function __livePollLoop() {
  try {
    const { url, data } = await __fetchLiveStats();
    if (LIVE_DEBUG) console.log("[LIVE] fetched from", url, data);

    // Some workers may return { error: "missing mint" } or similar
    if (data && data.error) {
      if (LIVE_DEBUG) console.warn("[LIVE] worker error:", data.error);
      // don't crash the sim — just keep retrying
    } else {
      __applyLiveStats(data);
    }
  } catch (e) {
    if (LIVE_DEBUG) console.warn("[LIVE] poll failed:", e?.message || e);
  } finally {
    setTimeout(__livePollLoop, LIVE_POLL_MS);
  }
}

// Start polling AFTER the rest of the script has initialized.
// Using requestAnimationFrame avoids breaking early init order.
requestAnimationFrame(() => {
  setTimeout(__livePollLoop, 1000);
});

// Expose a small debug helper in console:
// window.__wormLive = { get buyers(){return buyers}, get lastTotalBuys(){return __liveLastTotalBuys} }
try {
  window.__wormLive = {
    get buyers() { return buyers; },
    get lastTotalBuys() { return __liveLastTotalBuys; },
    base: LIVE_WORKER_BASE,
    mint: LIVE_TOKEN_MINT
  };
} catch (_e) {}

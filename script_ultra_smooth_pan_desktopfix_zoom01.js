...
// ===== PAN_POINTER_FIX (desktop + mobile) =====
// Drag to pan (works on desktop mouse and touch). Wheel zoom already handled elsewhere.
let __dragging = false;
let __dragStartX = 0, __dragStartY = 0;
let __dragStartCamX = 0, __dragStartCamY = 0;

function __getCanvasEl() {
  return document.querySelector("canvas");
}

function __toWorldDelta(dx, dy) {
  // convert screen delta to world delta based on zoom
  const z = zoom || 1;
  return { wx: dx / z, wy: dy / z };
}

(function attachPanHandlers(){
  function findCanvas(){
    // Prefer explicit canvas if you have an id; otherwise pick the first canvas
    return document.getElementById("c") || document.querySelector("canvas");
  }

  function install(c){
    if (!c || c.__panInstalled) return false;
    c.__panInstalled = true;

    // Prevent browser gestures / text selection from hijacking drags
    ...
    // Use BOTH pointer + mouse events for max desktop compatibility
    ...
    c.addEventListener("pointermove", (e) => {
      ...
      const { wx, wy } = toWorldDelta(dx, dy);
      camX = startCamX - wx;
      camY = startCamY - wy;
      e.preventDefault();
    });
    ...
    // Fallback mouse events (some browsers/overlays can block pointer capture)
  ...
  // Try now, then on DOM ready, then observe for late-created canvas
...
function updateBossCamera(dt) {
  ...
  canvas.addEventListener("touchmove", (e) => {
    ...
    zoom = clamp(pinchStartZoom * scale, 0.1, 2.8);
...
let __sync = {
  ...
  function steerAngle(current, desired, gain, dt, maxRate, deadzone){
    // Robust, frame-rate independent steering with deadzone + max turn rate.
    let d = wrapAngle(desired - current);
    if (Math.abs(d) < (deadzone || 0)) return current;
    const maxStep = (maxRate || 4.0) * Math.max(0.001, dt);
    if (d > maxStep) d = maxStep;
    if (d < -maxStep) d = -maxStep;
    return wrapAngle(current + d * gain);
  }
  ...
  function wormBehavior(col, w, time, dt) {
    ...
    if (w.mission?.type === "FOUND_COLONY") {
      ...
      head.a = steerAngle(head.a, desired, 0.65, dt, w.isBoss ? 5.0 : 4.0, 0.02);
    ...
    const gain = clamp(turnAmt * 6.0, 0.10, w.isBoss ? 0.28 : 0.22);
    const maxRate = w.isBoss ? 5.5 : 4.2; // rad/sec
    head.a = steerAngle(head.a, desired, gain, dt, maxRate, 0.02);
    // keep only a tiny low-frequency wobble (no per-frame random noise)
    head.a = wrapAngle(head.a + jitter * 0.08);
  ...
  bind("zoomIn", () => (zoom = clamp(zoom * 1.12, 0.1, 2.8)));
  bind("zoomOut", () => (zoom = clamp(zoom * 0.88, 0.1, 2.8)));
  bind("capture", () => {
    try {
